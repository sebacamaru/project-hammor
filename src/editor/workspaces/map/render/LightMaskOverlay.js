...
export class LightMaskOverlay {
  constructor() {
    ...
    /** @type {number} Ambient fill color (hex). */
    this._ambientColor = 0x000000;

    /** @type {number} Ambient fill alpha (0–1). */
  ...
  /**
   * Updates the ambient fill color and intensity of the darkness layer.
   * No-ops if values are unchanged. Triggers a re-render on next renderMask() call.
   * @param {number} color - Hex color (e.g. 0x223344).
   * @param {number} intensity - Alpha 0–1.
   */
  setAmbient(color, intensity) {
    const c = Number.isFinite(color) ? color : 0x000000;
    const a = Math.max(0, Math.min(1, intensity));
    if (this._ambientColor === c && this._ambientIntensity === a) return;
    this._ambientColor = c;
    this._ambientIntensity = a;
    this._rebuildDark();
  }

  /**
   * Redraws the dark background rectangle using current ambient color/intensity.
   * Skips drawing if intensity is zero (no visible rect). Marks the overlay dirty.
   */
  _rebuildDark() {
    this._darkGraphics.clear();
    if (this._mapW > 0 && this._mapH > 0 && this._ambientIntensity > 0) {
      this._darkGraphics.rect(0, 0, this._mapW, this._mapH);
      this._darkGraphics.fill({ color: this._ambientColor, alpha: this._ambientIntensity });
    }
    this._dirty = true;
  }
